...
class darkClustDens : public Clusterer<DomainType, ParticleDataType>
{
protected:
...
public:
    darkClustDens(std::ostream& output, size_t rank)
        : Base(output, rank)
    {
    ...
    {   
        ...
        domain.exchangeHalos(std::tie(get<"rho">(d)), get<"keys">(d), get<"keyBuf">(c));
        timer.step("mpi::synchronizeHalos");

        computeLocalDensityGroups(groups_.view(), d, c, domain.box());
        timer.step("LocalDensityGroups");
        pmReader.step();
        release(d, "rho");
